...
while not _G.ZundaVN and tick() < deadline do
	task.wait(0.15)
end
...
if zonesFolder then
	zonesFolder.DescendantAdded:Connect(function(desc)
		if desc:IsA("ClickDetector") then
			...
			local obj = desc
			while obj and obj.Parent ~= zonesFolder do
				obj = obj.Parent
			end
			if obj then
...
print(
	...
	(function()
		local n = 0
		for _ in pairs(zoneMap) do
			n = n + 1
		end
		return n
	end)(),
	"zone entrance ClickDetectors to ZundaVN"
)
