...
sub.Text = "Complete quests to earn Gold 💰"
...
local function buildCard(q, idx)
	...
	card.Name = "Card_" .. q.id
	card.Size = UDim2.new(1, 0, 0, 110)
	card.LayoutOrder = idx
	card.BackgroundColor3 = Color3.fromRGB(248, 244, 238)
	...
	local tt = Instance.new("TextLabel", card)
	...
	local dd = Instance.new("TextLabel", card)
	...
	local rb = Instance.new("Frame", card)
	rb.Size = UDim2.new(0, 58, 0, 28)
	rb.Position = UDim2.new(1, -68, 0, 12)
	rb.BackgroundColor3 = Color3.fromRGB(255, 242, 180)
	...
	rl.Text = "💰 " .. (q.reward_gold or q.reward or 0)
	rl.Font = Enum.Font.GothamBold
	rl.TextSize = 13
	rl.TextColor3 = Color3.fromRGB(160, 120, 30)

	-- Checkbox for completed quests (hidden by default)
	local checkBtn = Instance.new("TextButton", card)
	checkBtn.Name = "CheckBtn"
	checkBtn.Size = UDim2.new(0, 58, 0, 28)
	checkBtn.Position = UDim2.new(1, -68, 0, 74)
	checkBtn.BackgroundColor3 = Color3.fromRGB(100, 185, 100)
...
local function updateCards(quests, progress)
	for _, q in ipairs(quests) do
		...
		local w = cards[q.id]
		local p = progress and progress[q.id]
		if w and p then
			...
			if p.done then
				w.cs.Color = C.done
				w.card.BackgroundColor3 = Color3.fromRGB(235, 250, 235)
				w.em.Text = "✅"
...
if ev then
	ev.OnClientEvent:Connect(updateCards)
end
	...
	qcBatch.OnClientEvent:Connect(function(completions)
		for _, q in ipairs(completions) do
			...
			UIHelper.spawnSparkles(
				panel,
				panel.AbsolutePosition.X + 200,
				panel.AbsolutePosition.Y + 60,
...
spawnPopup = function(kind, text, color)
	...
	local bs = Instance.new("UIStroke", lbl)
	bs.Color = Color3.fromRGB(180, 150, 110)
	bs.Thickness = 2
...
task.spawn(function()
	...
	if h then
		local b = h:WaitForChild("HudButtons", 8)
		if b then
			local btn = b:FindFirstChild("HudBtn_quests")
			if btn then
