...
ComboUpdate.OnClientEvent:Connect(function(count, multiplier)
	if count <= 0 then
		TweenService
			:Create(combo, TweenInfo.new(0.3), { Position = UDim2.new(0.5, -110, 0, 60), BackgroundTransparency = 1 })
			:Play()
	...
	TweenService
		:Create(combo, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = origSize })
		:Play()
...
LevelUpEvent.OnClientEvent:Connect(function(level, tierName, tierColor, tierBadge)
	...
	bs.Color = tierColor or UIConfig.GAME_COLORS.SparkleGold
	bs.Thickness = 3
	bs.Parent = banner
...
if DailyUpdate and dTitle and dDesc then
	DailyUpdate.OnClientEvent:Connect(function(q, progress, claimed)
		...
		if claimed and dFill then
			dFill.BackgroundColor3 = UIConfig.COLORS.Success
		end
	end)
end

if LoginBonusEvent then
	LoginBonusEvent.OnClientEvent:Connect(function(streak, bonus, q)
		...
		bc.Parent = banner
		local bs = Instance.new("UIStroke")
		bs.Color = UIConfig.GAME_COLORS.SparkleGold
		bs.Thickness = 3
		bs.Parent = banner
		local lines = {
		...
		TweenService:Create(banner, TweenInfo.new(0.4, Enum.EasingStyle.Back), { Size = UDim2.new(0, 480, 0, 140) })
			:Play()
...
AchievementUnlocked.OnClientEvent:Connect(function(name, desc, icon)
	...
	ts.Color = UIConfig.GAME_COLORS.SparkleGold
	ts.Thickness = 2
	ts.Parent = toast
	...
	ico.TextScaled = true
	ico.TextColor3 = UIConfig.GAME_COLORS.SparkleGold
	ico.Parent = toast

	local t1 = Instance.new("TextLabel")
	t1.Size = UDim2.new(1, -72, 0, 28)
	t1.Position = UDim2.new(0, 72, 0, 6)
	t1.BackgroundTransparency = 1
	t1.Text = "🏆 " .. name
	...
	local t2 = Instance.new("TextLabel")
	t2.Size = UDim2.new(1, -72, 0, 24)
	t2.Position = UDim2.new(0, 72, 0, 38)
	t2.BackgroundTransparency = 1
	t2.Text = desc
	t2.Font = Enum.Font.Gotham
	t2.TextScaled = true
	t2.TextXAlignment = Enum.TextXAlignment.Left
	t2.TextColor3 = UIConfig.COLORS.TextDisabled
	t2.Parent = toast

	local myOffset = toastY
	toastY = toastY + 80
	TweenService
		:Create(toast, TweenInfo.new(0.35, Enum.EasingStyle.Back), { Position = UDim2.new(1, -360, 0, 200 + myOffset) })
		:Play()
	task.delay(4.0, function()
		TweenService:Create(toast, TweenInfo.new(0.4), { Position = UDim2.new(1, 360, 0, 200 + myOffset) }):Play()
		task.wait(0.5)
		toast:Destroy()
		toastY = math.max(0, toastY - 80)
	end)
end)
...
pCorner.Parent = progressPanel
local pStroke = Instance.new("UIStroke")
pStroke.Color = C.accent
pStroke.Thickness = 3
pStroke.Parent = progressPanel

local pTitle = Instance.new("TextLabel")
pTitle.Size = UDim2.new(1, 0, 0, 44)
pTitle.BackgroundTransparency = 1
pTitle.Text = "📊  Chef Progress"
pTitle.Font = Enum.Font.FredokaOne
pTitle.TextSize = 28
pTitle.TextColor3 = C.text
pTitle.Parent = progressPanel

local pClose = Instance.new("TextButton")
pClose.Size = UDim2.new(0, 38, 0, 38)
pClose.Position = UDim2.new(1, -46, 0, 6)
pClose.Text = "✕"
pClose.Font = Enum.Font.FredokaOne
pClose.TextSize = 16
pClose.BackgroundColor3 = C.red
pClose.TextColor3 = C.white
...
local function renderTab(name)
	...
	elseif name == "mastery" then
		...
		if i == 0 then
			...
			l.BackgroundTransparency = 1
			l.Text = "Cook a recipe to start earning mastery!"
	...
	elseif name == "tools" then
		for j, toolKey in ipairs({ "Axe", "PickAxe", "Sickle" }) do
			...
			if tier < 3 then
				...
				btn.Size = UDim2.new(0.28, -8, 0, 44)
				btn.Position = UDim2.new(0.72, 0, 0.5, -22)
				btn.Text = "Upgrade  " .. cost .. "g"
	...
	elseif name == "powerups" then
		...
		for key, cfg in pairs(data.powerupConfig) do
			...
			label.TextColor3 = active and C.green or C.text
			label.TextWrapped = true
			label.Parent = row
			local btn = Instance.new("TextButton")
			btn.Size = UDim2.new(0.28, -8, 0, 50)
			btn.Position = UDim2.new(0.72, 0, 0.5, -25)
			btn.Text = "Use  " .. (cfg.cost.gold or "?") .. "g"
			btn.Font = Enum.Font.FredokaOne
			btn.TextSize = 14
