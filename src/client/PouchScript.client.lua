...
do
	...
	local st = Instance.new("UIStroke")
	st.Thickness = 1.5
	st.Color = Color3.fromRGB(220, 180, 50)
	st.Parent = goldLbl
end
...
local function render(tab)
	...
	for _, c in ipairs(scroll:GetChildren()) do
		if c:IsA("Frame") and c.Name ~= "_Empty" then
			c:Destroy()
		end
	end
	...
	countLbl.Text = shown .. (shown == 1 and " item" or " items") .. (tab ~= "all" and (" in " .. tab) or " in pouch")
end
...
UIRouter.register("inventory", nil, function()
	shell.close()
end)
