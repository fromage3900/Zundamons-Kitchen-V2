-- [[ModuleScript] Type (ref: RBX5519E59A845C479480E4F61F6E5FD7D2)]]
...
function lib.isALibrary(value)
	...
	return type(value) == "table" and type(value.GetApi) == "function"
end
...
function lib.coerceIntoEnum(value, enum)
	...
	if lib.isAnEnum(enum) then
		for _, enum_item in next, enum:GetEnumItems() do
			if value == enum_item or value == enum_item.Name or value == enum_item.Value then
				return enum_item
			end
		end
	else
...
function lib.getType(value)
	...
	if valueType == "userdata" then
		if lib.isAnInstance(value) then
			return value.ClassName
		elseif lib.isAColor3(value) then
			return "Color3"
		elseif lib.isACoordinateFrame(value) then
			return "CFrame"
		elseif lib.isABrickColor(value) then
			return "BrickColor"
		elseif lib.isAUDim2(value) then
			return "UDim2"
		elseif lib.isAUDim(value) then
			return "UDim"
		elseif lib.isAVector3(value) then
			return "Vector3"
		elseif lib.isAVector2(value) then
			return "Vector2"
		elseif lib.isARay(value) then
			return "Ray"
		elseif lib.isAnEnum(value) then
			return "Enum"
		elseif lib.isASignal(value) then
			return "RBXScriptSignal"
		elseif lib.isALibrary(value) then
			return "RbxLibrary"
		elseif lib.isAAxis(value) then
			return "Axes"
		elseif lib.isAFace(value) then
			return "Faces"
		end
	else
		return valueType
