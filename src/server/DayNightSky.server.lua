...
pcall(function()
	Lighting.ExtendLightRangeTo120 = true
end)

-- ── ATMOSPHERE + SKY ──────────────────────────────────────
for _, c in ipairs(Lighting:GetChildren()) do
	if c:IsA("Atmosphere") or c:IsA("Sky") or c:IsA("BlurEffect") then
		c:Destroy()
	end
end
...
if CONFIG.sky.skybox_bk then
	sky.SkyboxBk = CONFIG.sky.skybox_bk
end
if CONFIG.sky.skybox_dn then
	sky.SkyboxDn = CONFIG.sky.skybox_dn
end
if CONFIG.sky.skybox_ft then
	sky.SkyboxFt = CONFIG.sky.skybox_ft
end
if CONFIG.sky.skybox_lf then
	sky.SkyboxLf = CONFIG.sky.skybox_lf
end
if CONFIG.sky.skybox_rt then
	sky.SkyboxRt = CONFIG.sky.skybox_rt
end
if CONFIG.sky.skybox_up then
	sky.SkyboxUp = CONFIG.sky.skybox_up
end
sky.Parent = Lighting
...
local function updateSkybox(hour)
	local set
	if (hour >= 5 and hour < 7.5) or (hour >= 17 and hour < 19.5) then
...
if godRayFolder then
	godRayFolder:Destroy()
end
...
local function updateClouds(hour, weather)
	...
	if isNight and (weather == "clear" or weather == "cherry_blossom") then
		baseCover = 0.08
	end
