...
local function rollWeather()
	...
	for _, entry in ipairs(pool) do
		if entry.weather ~= "aurora" or isNight then
...
local function applyWeather(weatherKey, transitionSec)
	...
	task.spawn(function()
		...
		while tick() - t0 < transitionSec do
			...
			workspace.GlobalWind = startWind:Lerp(targetWind, p)
			task.wait(0.15)
		end
...
shared.ZundaWeather = {
	set = function(w, dur)
		applyWeather(w, dur or CONFIG.weather.transition_seconds)
	end,
	get = function()
	...
	list = function()
		local out = {}
		for k in pairs(CONFIG.weather_types) do
			table.insert(out, k)
		end
		return out
	end,
