...
-- os.getenv is supplied by modern Roblox Luau (re-added 2024); selene 0.27's Lua std predates it.
...
function DiscordWebhookService.broadcastGachaPull(player: Player, itemName: string, rarity: string, bannerName: string)
	if rarity ~= "5star" and rarity ~= "SSR" and rarity ~= "Legendary" then
		return
	end
	...
	local embed = {
		...
		description = string.format(
			"**%s** just pulled **[%s]** from the *%s* banner nanoda! 🌱✨",
			player.Name,
			itemName,
			bannerName or "Whim Banner"
		),
		...
		fields = {
			...
			{
				name = "📊 Web Hub",
				value = "[Zunda-OS 95 Portal](https://fromage3900.github.io/Zundamons-Kitchen-V2/)",
				inline = true,
			},
		},
...
function DiscordWebhookService.broadcastChallengeWave(player: Player, waveNumber: number, score: number)
	...
	local embed = {
		...
		description = string.format(
			"Chef **%s** reached **Wave %d** in Challenge Mode with **%d points** nanoda! 🔥",
			player.Name,
			waveNumber,
			score
		),
...
function DiscordWebhookService.broadcastBossDefeat(player: Player, bossName: string)
	local embed = {
		...
		description = string.format(
			"Master Chef **%s** has defeated the Weekly Boss **[%s]** nanoda! 🎉",
			player.Name,
			bossName
		),
