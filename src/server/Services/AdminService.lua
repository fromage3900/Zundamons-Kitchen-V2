...
function AdminService.executeCommand(player, input)
	if typeof(input) ~= "string" or input:sub(1, 1) ~= "/" then
		return
	end
	if not isAdmin(player) then
		warn(player.Name .. " tried admin command without permission")
		return
	end
...
AdminService.registerCommand("giveitem", function(player, args)
	...
	if not itemName then
		return "Usage: /giveitem <name> [count]"
	end
	local data = PlayerDataService.getOrCreate(player)
...
AdminService.registerCommand("scatter", function(player, args)
	local success, svc = pcall(function()
		return require(game.ServerScriptService.Services.ScatterService)
	end)
	if not success or not svc or not svc.scatterBiome then
		return "ScatterService not available"
	end
...
AdminService.registerCommand("listplayers", function()
	...
	for _, p in ipairs(Players:GetPlayers()) do
		...
		lines[#lines + 1] = string.format(
			"  %s (gold: %d, served: %d)",
			p.Name,
			data and data.gold or 0,
			data and data.guests_served or 0
		)
	end
	return table.concat(lines, "\n")
...
AdminService.registerCommand("applymod", function(player, args)
	...
	if not modName then
		return "Usage: /applymod <modifier_name> [target]"
	end
