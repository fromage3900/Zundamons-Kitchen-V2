...
local function handleLogin(player)
	...
	if d.daily.lastClaimDay == today then
		return
	end
...
local function gatherMetrics(d)
	local maxRep = 0
	for _, v in pairs(d.guestRep) do
		if v > maxRep then
			maxRep = v
		end
	end
	...
	for _, m in pairs(d.mastery) do
		if m.level and m.level > maxMastery then
			maxMastery = m.level
		end
		if m.level and m.level >= 5 then
			masteredCount = masteredCount + 1
		end
	end
	local maxToolTier = 1
	for _, v in pairs(d.toolTiers) do
		if v > maxToolTier then
			maxToolTier = v
		end
	end
	return {
...
local function setMaxStat(d, name, value)
	d.stats = d.stats or {}
	if (d.stats[name] or 0) < value then
		d.stats[name] = value
	end
end
...
local function progressDaily(player, metric, amount)
	local d = PlayerDataService.get(player)
	if not d.daily.todayQuestId or d.daily.todayClaimed then
		return
	end
	local q
	for _, p in ipairs(DailyQuestConfig.pool) do
		if p.id == d.daily.todayQuestId then
			q = p
			break
		end
	end
	if not q or q.metric ~= metric then
		return
	end
		...
		if payload.recipe then
			...
			if m.xp >= m.level * 5 and m.level < 10 then
				...
				PopupEvent:FireClient(
					player,
					"bonus",
					"📖 " .. payload.recipe .. " mastery " .. m.level,
				...
				if m.level == 5 then
					PopupEvent:FireClient(
						player,
						"bonus",
						"🌟 Perk: bonus dish +10% for " .. payload.recipe,
				...
				elseif m.level == 10 then
					PopupEvent:FireClient(
						player,
						"bonus",
						"🏆 Perk: gold +15% for " .. payload.recipe,
...
UsePowerup.OnServerInvoke = function(player, key)
	...
	if isSpamming(player, "UsePowerup", 0.5) then
	...
	local cfg = PowerupConfig[key]
	if not cfg then
		return false, "unknown powerup"
	end

	local cost = cfg.cost and cfg.cost.gold or 0
	if (d.gold or 0) < cost then
...
UpgradeTool.OnServerInvoke = function(player, toolType)
	if typeof(toolType) ~= "string" then
		return false, "invalid toolType"
	end
	if isSpamming(player, "UpgradeTool", 1.0) then
		return false, "rate limit"
	end
	...
	if (d.gold or 0) < cost then
		return false, "not enough gold (need " .. cost .. ")"
	end
...
Players.PlayerAdded:Connect(function(player)
	player.CharacterAdded:Connect(function()
		...
		d.daily = d.daily
			or { lastClaimDay = 0, streak = 0, todayQuestId = nil, todayProgress = 0, todayClaimed = false }
