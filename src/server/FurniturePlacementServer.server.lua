...
if not purchaseRF then
	purchaseRF = Instance.new("RemoteFunction")
	purchaseRF.Name = "PurchaseFurniture"
	purchaseRF.Parent = RF
end
purchaseRF.OnServerInvoke = function(player, itemId)
	...
	if not d.furniture_unlocked then
		d.furniture_unlocked = {}
	end
	if table.find(d.furniture_unlocked, itemId) then
		return false, "Already owned"
	end
	if (d.gold or 0) < (item.price or 0) then
		return false, "Need " .. item.price .. " gold"
	end
...
if not placeRF then
	placeRF = Instance.new("RemoteFunction")
	placeRF.Name = "PlaceFurniture"
	placeRF.Parent = RF
end
placeRF.OnServerInvoke = function(player, itemId, posX, posY, posZ, rotX, rotY, rotZ)
	...
	if not d.owned_plot then
		return false, "No plot owned"
	end
	local item = findItem(itemId)
	if not item then
		return false, "Item not found"
	end
	if not d.furniture_unlocked or not table.find(d.furniture_unlocked, itemId) then
		return false, "Not owned"
	end
	if not d.placed_furniture then
		d.placed_furniture = {}
	end
	if #d.placed_furniture >= 10 then
		return false, "Max 10 items placed"
	end
	if typeof(posX) ~= "number" then
		return false, "Invalid position"
	end
	local pos = Vector3.new(posX, posY, posZ)
	local rot = Vector3.new(rotX or 0, rotY or 0, rotZ or 0)
	if (pos - Vector3.new(155, -508, -430)).Magnitude > 30 then
		return false, "Too far from plot area"
	end
...
if not removeRF then
	removeRF = Instance.new("RemoteFunction")
	removeRF.Name = "RemoveFurniture"
	removeRF.Parent = RF
end
removeRF.OnServerInvoke = function(player, index)
	...
	if not d.placed_furniture or not d.placed_furniture[index] then
...
if not getRF then
	getRF = Instance.new("RemoteFunction")
	getRF.Name = "GetPlacedFurniture"
	getRF.Parent = RF
end
