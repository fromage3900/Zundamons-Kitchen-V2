...
if not qcBatch then
	qcBatch = Instance.new("RemoteEvent")
	qcBatch.Name = "QuestCompletedBatch"
	qcBatch.Parent = RE
end
-- Side dialogue trigger (client listens for this)
local sideDlgRE = RE:FindFirstChild("TriggerSideDialogue")
if not sideDlgRE then
	sideDlgRE = Instance.new("RemoteEvent")
	sideDlgRE.Name = "TriggerSideDialogue"
	sideDlgRE.Parent = RE
end
...
local function triggerSideDialogue(player, key)
	...
	pcall(function()
		sideDlgRE:FireClient(player, key)
	end)
end
...
local function questCheck(d, q, player)
		...
		if q.target_item then
			...
			for k, v in pairs(d) do
				if
					type(v) == "number"
					and k ~= "gold"
					and k ~= "total_gold_earned"
					and k ~= "guests_served"
					and k ~= "perfect_cooks"
					and k ~= "great_cooks"
					and k ~= "companion_affection"
					and k ~= "companion_chats"
					and k ~= "cooking_streak"
					and k ~= "max_cooking_streak"
					and k ~= "tier"
					and k ~= "recipes_unlocked_count"
				then
					count = count + v
				end
			end
			...
			for _, r in pairs(QuestConfig.quests) do
				if r.type == "cook" and r.target_item and d[r.target_item] then
					count = count - (d[r.target_item] or 0)
				end
			end
			cur = 0
		end
	...
	elseif q.type == "cook_unique_zunda" then
		...
		for name in pairs(rsc) do
			if zunda_recipes[name] then
				cur = cur + 1
			end
		end
...
local function eval(player)
	...
	for _, q in ipairs(QUESTS) do
		...
		if cur >= goal then
			if not d.quests_completed then
				d.quests_completed = {}
			end
			if not d.quests_completed[q.id] then
				...
				if q.chain_id then
					if not d.chains_progress then
						d.chains_progress = {}
					end
					if not d.chains_progress[q.chain_id] then
						d.chains_progress[q.chain_id] = {}
					end
					...
					if allDone then
						if not d.chains_completed then
							d.chains_completed = {}
						end
...
if not chatEv then
	chatEv = Instance.new("RemoteEvent")
	chatEv.Name = "CompanionChat"
	chatEv.Parent = RE
end
...
if not npcChat then
	npcChat = Instance.new("RemoteEvent")
